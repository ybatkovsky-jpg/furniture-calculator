...
### Промпт v2.3: измерение высот + без ложных углов (2026-09-06, ночь)
- **Высоты читаются с чертежа**: ЗАДАЧА 1Б в промпте — модель возвращает опциональный `heights_mm: {"lower","upper","penal"}` (вертикальные размерные линии рядов); `analyze_page` передаёт их в `scale_calc` (раньше были только константы 716/596/2500). Кламп абсурда 200–2900 мм. Верифицировано: 0002 → lower=895, upper=1485, penal=2483; пеналы на других стр. — 2388–2483 (реальные значения).
- **Авто-угол убран**: эвристика «нижний фасад 800–1100мм = corner» удалена — угол только по явному `is_corner` от модели (устранило ложные «угловые» TV-тумбы 0004 и шкафа 0006). Тесты обновлены, суит **258 passed**.
- Боевой прогон альбома v2.3: `output/Расчет_по_картинкам_v23.xlsx`; вариативность модели остаётся (0011: ширина 1581↔2581, зона «Кухня»; 0004: 1830↔3000) — главный оставшийся источник ошибок.
- **Excel-видимость**: лист «🧩 Модули (распознано)» + блок «🧩 Мебель:» в шапке каждого листа-комнаты (что именно посчитано).

...
- УГЛОВАЯ (Г-образная) кухня в РАЗВОРОТЕ: фасады вытянуты в ОДНУ непрерывную линию разворота. НЕ складывай длины двух плеч и НЕ удваивай габарит — бери ОДНО число общей длины разворота.
...
- Если видишь несколько размерных линий — бери ту, что ДЛИННЕЕ всех, но НЕ складывай их
- Если кухня и число вышло >8000мм — ты наверняка сложил два плеча угловой кухни или две размерные линии: перечитай ОДНУ линию (общую длину разворота)
        ...
        try:
            ...
            if settings.ocr_dimensions_enabled and total_width_mm > 0 and facades_data:
                try:
                    ...
                    # Достоверность OCR: (а) уверенное чтение, либо
                    # (б) модель дала нереалистичный габарит (>8000мм) — тогда
                    # доверяем крупному OCR-числу (≥1000мм). Это чинит
                    # «удвоение» угловой кухни в развороте (9360 → 4860).
                    ocr_trustworthy = (
                        ocr_width is not None
                        and (
                            ocr_conf >= 0.7
                            or (total_width_mm > 8000 and ocr_width >= 1000)
                        )
                    )
                    if ocr_trustworthy and abs(ocr_width - total_width_mm) > 10:
                        logger.info(
